...
app.get("/user/tweets/feed/", authToken, async (request, response) => {
    ...
    const id_of_the_logged_in_user = await db.get(getUserIdQuery);
    const tweetsQuery = `
        SELECT
        ...
        WHERE
           follower.follower_user_id = ${id_of_the_logged_in_user.user_id}
        ORDER BY
...
app.get("/user/following/", authToken, async (request, response) => {
    ...
    const userFollowing = `
        ...
        FROM 
           follower 
        INNER JOIN user 
            ON follower.follower_id = ${id_of_the_logged_in_user.user_id}`;
...
app.get("/user/followers/", authToken, async (request, response) => {
    ...
    const followerUser = `
        ...
        FROM 
           follower
        INNER JOIN user 
            ON follower.follower_user_id =  ${id_of_the_logged_in_user.user_id}
        WHERE 
    ...
    const selectRequest = `
        SELECT 
         ...
         FROM 
           follower 
        INNER JOIN tweet 
          ON tweet.tweet_id = ${id_of_the_logged_in_user.user_id}
        INNER JOIN like
           ON like.like_id = ${id_of_the_logged_in_user.user_id}
        INNER JOIN reply 
            ON reply.reply_id = ${id_of_the_logged_in_user.user_id}`;
    const Result = await db.all(selectRequest);
    if (Result !== undefined) {
        response.status(200);
        response.send(Result);
    } else {
...
app.get("/user/tweets/", authToken, async (request, response) => {
    ...
    const SelectTweets = `
       SELECT   
        ...
        INNER JOIN reply 
            ON reply.user_id = ${id_of_the_logged_in_user.user_id}`;
